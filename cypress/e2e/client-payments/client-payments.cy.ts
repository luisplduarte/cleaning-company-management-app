describe('Client Payments Management', () => {
  beforeEach(() => {
  ...
  describe('Client Payments Access and Management with Admin Access', () => {
    beforeEach(() => {
      // Login as admin to access client payments management features
      cy.login('l.duarte@runtime-revolution.com', 'Runtime0!');
    });

    it('should access client payments page and display payments table', () => {
      // Navigate to client payments page
      cy.visit('/client-payments');
      cy.url({ timeout: 10000 }).should('include', '/client-payments');
      
      // Wait for page to fully load - look for the page container instead of main
      cy.get('div', { timeout: 10000 }).should('be.visible');
      
      // Verify page header is present
      cy.contains('Client Payments', { timeout: 15000 }).should('be.visible');
      
      // Verify description is present
      cy.contains('Track and manage payment records. The status can be updated to reflect the current state of each payment.', { timeout: 10000 }).should('be.visible');
    ...
    it('should load and display client payments data from API', () => {
      // Intercept the API call to check data loading
      cy.intercept('GET', '/api/client-payments').as('getClientPayments');
      
      // Navigate to client payments page
      cy.visit('/client-payments');
      
      // Wait for API call to complete
      cy.wait('@getClientPayments', { timeout: 15000 }).then((interception) => {
    ...
    it('should display edit button for payment status updates', () => {
      ...
      cy.get('tbody tr').then(($rows) => {
        if ($rows.length > 0) {
          ...
          // If no payments, that's also valid for testing
    ...
    it('should successfully update payment status via API', () => {
      ...
      cy.get('tbody tr').then(($rows) => {
        if ($rows.length > 0) {
          ...
          cy.intercept('PATCH', '/api/client-payments/*').as('updatePaymentStatus');
    ...
    it('should handle API errors gracefully during status update', () => {
      ...
      cy.get('tbody tr').then(($rows) => {
        if ($rows.length > 0) {
          ...
          cy.intercept('PATCH', '/api/client-payments/*', { 
            statusCode: 500, 
    ...
    it('should display proper loading states', () => {
      ...
      cy.intercept('GET', '/api/client-payments', { 
        delay: 1000,
        body: []
      }).as('getClientPaymentsWithDelay');
      ...
      // Wait for API call to complete
      cy.wait('@getClientPaymentsWithDelay', { timeout: 15000 });
    ...
    it('should handle navigation back from client payments page', () => {
      // Navigate to client payments page
      cy.visit('/client-payments');
      ...
      // Navigate back to client payments
      cy.visit('/client-payments');
      cy.url({ timeout: 10000 }).should('include', '/client-payments');
      
      // Verify page loads correctly again
      cy.contains('Client Payments', { timeout: 10000 }).should('be.visible');
      cy.get('table', { timeout: 10000 }).should('be.visible');
    });
  });
});
